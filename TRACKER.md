...
draws it — and pulling that thread found an actual bug, because **nothing in
this engine loads an authored normal map.** Every one is generated by our own
code: `tools/gen-testmap-textures.py` for the test sheets,
`R_GenerateNormalMap` for the R20 maps derived from a diffuse at run time. The
convention was never a coin flip. It was a fact about two functions, and they
disagreed.
...
`r_qlNormalFlipG` stays, because a genuinely empirical question remains: does
our now-shared convention match the way V actually runs on a surface? One look
at the dome panel settles it, and then it becomes a constant.
...
**Still unverified:** whether 0 or 1 is right. `/devmap qltest_bump`, dome
panel, `r_qlNormalFlipG` 0 vs 1 with a `vid_restart` between.
