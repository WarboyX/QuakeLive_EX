...
### E29. We shared a namespace with the Steam client — DONE (verify)
**Lives in:** our **client engine** + **server engine** · **Seen by:** stock Steam Quake Live

Reported as Steam Quake Live compatibility breaking, with `repconfig`/`qzconfig`
suspected. That was the right thread to pull.

Our engine wrote **`qzconfig.cfg`** and **`repconfig.cfg`** — QL's own filenames —
carrying QL's own header text (`Generated by QUAKE LIVE`), into
`fs_homepath`, which defaulted to `%APPDATA%\quakelive` / `~/.quakelive` — QL's
own home directory name.

Sharing a filename is only safe while the two never share a directory, and
nothing enforces that. `fs_homepath` is a cvar; our build is normally unpacked
into a copy of a Quake Live install; point it at that install and our config
lands exactly where the Steam client reads its own. It then execs a file full of
our cvars — the Vulkan renderer's `r_*` set, everything we have added that it has
never heard of, and our values for the ones it shares.

The config is not even the worst of it. The engine extracts the game modules from
`iobin.pk3` into `fs_homepath/baseq3` under the stock names — `cgamex86_64.dll`
and friends — so a shared homepath means the Steam client can load **our**
cgame/qagame/ui. That is the failure worth designing out rather than documenting
around.

Fixed by giving both a namespace of our own:

| | was | now |
|---|---|---|
| home directory (Win) | `%APPDATA%\quakelive` | `%APPDATA%\ioquakelive` |
| home directory (Unix) | `~/.quakelive` | `~/.ioquakelive` |
| client config | `qzconfig.cfg` | `ioquakelive.cfg` |
| replicated config | `repconfig.cfg` | `ioquakelive_rep.cfg` |
| dedicated config | `server.cfg` | `ioquakelive_server.cfg` |
| header | `Generated by QUAKE LIVE` | `Generated by ioquakelive` |

`FS_CopyFromSteam` still imports the Steam user's configs on first run, but
copies them **under our names** — the source is opened read-only, which it always
was, and the destination is now ours. `autoexec.cfg` keeps its name: it is not a
generated config, it is the user's own script, and both clients read it by that
name. The import runs once, when our config does not exist yet, so it is a
starting point and not a link.

`FS_FOpenFileRead`'s "a config never comes out of a pk3" guard now lists QL's
names as well as ours — a pak shipping either one is a pak trying to overwrite
user settings.

**Note on upgrading:** settings from a previous build live in the old
`quakelive` home directory and are not migrated. On Windows the Steam import will
re-seed them; otherwise they are a copy-and-rename away
(`qzconfig.cfg` → `ioquakelive.cfg`, `repconfig.cfg` → `ioquakelive_rep.cfg`).

**What I could not verify from here:** whether a stock Steam Quake Live install
was actually damaged, and if so which of the two paths did it. The container has
