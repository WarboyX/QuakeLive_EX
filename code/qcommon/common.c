...
void Com_WriteConfigToFile(const char* filename) {
    ...
    FS_Printf(f, "// Hardware cfg - Generated by ioquakelive. Do not modify\n");
    Key_WriteBindings(f);
    Cvar_WriteVariables(f, f, 0);
    Cmd_WriteAliases(f);
    FS_FCloseFile(f);
}
...
void Com_WriteSplitConfig(const char* hwName, const char* repName) {
    ...
    // [QL] our own banner. The header used to claim QUAKE LIVE wrote these, which
    // was true of the format and false of the file, and made a stray config
    // impossible to attribute when one turned up in the wrong folder.
    FS_Printf(f_hw, "// Hardware cfg - Generated by ioquakelive. Do not modify\n");
    FS_Printf(f_rep, "// Replicate cfg - Generated by ioquakelive. Do not modify\n");

    // [QL] bindings go to repconfig, not qzconfig
    Key_WriteBindings(f_rep);

    // [QL] cvars are split between the two files
    Cvar_WriteVariables(f_hw, f_rep, 0);
...
void Com_WriteClientConfig_f(void) {
    ...
    FS_Printf(f, "// Hardware cfg - Generated by ioquakelive. Do not modify\n");
    Key_WriteBindings(f);
    // [QL] Address: 0x004cb630 - writeClientConfig writes only the userinfo subset
    // (CVAR_USERSAVE / 0x80000 cvars that differ from reset), not the full archive.
    Cvar_WriteVariables(f, f, 1);
    FS_FCloseFile(f);
}
