...
/*
[QL] E103. r_qlNormalFlipG: the green-channel convention.

The usual framing is "OpenGL convention versus DirectX convention", and that is
misleading on a Vulkan renderer: it describes how a texture was AUTHORED, not
which API draws it. Vulkan shares DirectX's top-left texture origin and flips
clip-space Y, and none of that touches tangent-space normals.

TWO SEPARATE QUESTIONS live under this name, and conflating them cost a round.

1. Do OUR OWN generators agree with each other? That is a fact about two
   functions, not a convention choice, and they did not agree.
   tools/gen-testmap-textures.py encoded (-dh/du, +dh/dv, 1) where the correct
   tangent-space normal for a height field is (-dh/du, -dh/dv, 1);
   R_GenerateNormalMap, which derives a map from any diffuse at run time, had
   the sign right. Every sheet the generator makes shared the error, so they
   agreed with each other and the disagreement only showed against a derived
   map. That is why the dome panel read as craters. Fixed in the generator.

2. What convention does an AUTHORED map use? This is the real coin flip and it
   does not go away. The shader parser accepts normalMap, bumpMap,
   normalParallaxMap and bumpParallaxMap, so any third-party Quake Live map can
   ship a normal map baked in whichever convention its artist's tool used. Our
   own tree happens to contain none today, which is exactly what made it easy to
   convince myself the question was settled.

So this switch is NOT a temporary diagnostic to be folded into a constant once
the dome panel is looked at. It has to stay for as long as the engine can load
somebody else's normal map, which is always.

The right long-term shape is per-material rather than global - a shader keyword
beside normalMap, so a map declares its own convention - with this cvar as the
global default. Not done yet, and the obstacle is named: the dynamic light
pipelines are built once at init, so a per-stage value cannot reach them through
a specialization constant the way it can reach the draw-time bump pass.
*/
