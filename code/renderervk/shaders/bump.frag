...
The usual framing for this is "OpenGL convention versus DirectX convention",
which is misleading here twice over. It describes how a texture was AUTHORED,
not which API draws it - and this renderer is Vulkan, which shares DirectX's
top-left texture origin and flips clip-space Y, none of which touches
tangent-space normals at all.

More to the point, nothing in this engine loads an authored normal map. Every
one is generated by our own code: tools/gen-testmap-textures.py for the test
sheets, R_GenerateNormalMap for the R20 maps derived from a diffuse at run time.
So the convention is not a coin flip, it is a fact about two functions - and
they disagreed. The generator encoded (-dh/du, +dh/dv, 1) where the correct
tangent-space normal for a height field is (-dh/du, -dh/dv, 1), while the C path
had the sign right. Every sheet the generator made shared the error, so they all
agreed with each other and the disagreement only showed against a derived map.
That is why the dome panel read as craters.

Both encoders now agree. This switch stays because the question it answers -
does OUR shared convention match the way V actually runs on a surface - is still
empirical, and one look at the dome panel settles it. If domes look like domes
at 0, this becomes a constant and goes away.
*/
